    ...
    {foreach $data as $entry}
        ...
        {if isset($entry.children)}
            <div class='row elementMenu' nom='{$entry.id}' style='margin-left:{($entry.lvl-1)*50}px'>
                ...
                {if $entry.lvl!=1}<div class='oneline deleteStruct'
                     onClick="location.href = '/structure/delete/{$entry.id}'"></div>{/if}
                {if $entry.visible}
        ...
        {else}
            <div class='row elementMenu {if $param==$entry.id}selected{/if}' nom='{$entry.id}' style='margin-left:{($entry.lvl-1)*50}px'>
                <div class='oneline addStruct' onClick="location.href = '/structure/add/{$entry.id}'"></div>
                <div class='oneline deleteStruct' onClick="location.href = '/structure/delete/{$entry.id}'"></div>
                {if $entry.visible}
                    <div class='oneline'><img src='/public/img/eyeBig.png' style='width:14px;height:14px;'></div>
                    {/if}
